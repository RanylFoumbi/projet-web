...
(function (global, factory) {
	...
	function unravel(signal, vars) {
	    ...
	    if (typeof signal === 'function') {
	        return signal(vue.toValue(vars));
	    }
	    return signal;
	}
	...
	function useQuery(opts, mapData = defaultMapper) {
	    ...
	    const data = vue.ref(defaultMapper({ data: null, error: null }));
	    ...
	    async function execute(overrideOpts) {
	        const vars = vue.toValue(variables) || {};
	        ...
	        const pendingExecution = client.executeQuery({
	            query: vue.toValue(query),
	            variables: vue.toValue((overrideOpts === null || overrideOpts === void 0 ? void 0 : overrideOpts.variables) || vars),
	        ...
	        }, vue.toValue(opts === null || opts === void 0 ? void 0 : opts.context), onResultChanged);
	    ...
	    function initVarWatchers() {
	        ...
	        vue.watch(() => vue.toValue(variables), newValue => {
	...
	function useMutation(query, opts) {
	    ...
	    async function execute(variables) {
	        ...
	        }, vue.toValue(opts === null || opts === void 0 ? void 0 : opts.context));
	        lastPendingOperation = pendingExecution;
	        const res = await pendingExecution;
	        vue.nextTick(() => {
	...
	function useSubscription(opts, reduce = defaultReducer) {
	    ...
	    const data = vue.ref((_c = opts === null || opts === void 0 ? void 0 : opts.initialData) !== null && _c !== void 0 ? _c : reduce({ data: null, error: null }, null));
	    const error = vue.ref(null);
	    const isPaused = vue.computed(() => unravel(paused, variables));
	    const isFetching = vue.ref(true);
	    ...
	    const subscribe = debounceAsync(async function subscribe() {
	        ...
	        const result = await client.executeSubscription({
	            query: vue.toValue(query),
	            variables: vue.toValue(variables),
	        });
	    ...
	    if (!isPaused.value && !shouldSkip() && subscribeOnMount) {
	        vm ? vue.onMounted(subscribe) : subscribe();
	    }
	    // TODO: if outside of setup, it should be recommend manually pause it(or some action else)
	    vm && vue.onBeforeUnmount(unsubscribe);
	    function shouldSkip() {
	        return unravel(skip, vue.toValue(variables) || {});
	    }
	    if (isWatchable(paused)) {
	    ...
	    if (isWatchable(query)) {
	        vue.watch(query, subscribe);
	    }
	    if (isWatchable(variables)) {
	        vue.watch(variables, (value, oldValue) => {
	    ...
	    if (isWatchable(skip)) {
	        vue.watch(shouldSkip, (value, oldValue) => {
	            if (value === oldValue) {
	                return;
	            }
